...
outlier.plot <- ggbetweenstats(df,group, Ik.log10, outlier.tagging = TRUE)
...
# get start vals
start_vals <- get_start_vals(df$Temp.Cat,df$Ik.log10, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(df$Temp.Cat,df$Ik.log10, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(df$Temp.Cat,df$Ik.log10, model_name = 'sharpeschoolhigh_1981')
...
#fit 
Dlab.fit <- nls_multstart(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Dlab.df,
                                                     iter = 500,
...
Dlab.fit_nlsLM <- minpack.lm::nlsLM(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Dlab.df,
                        start = coef(Dlab.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Dlab.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Ik.log10), Dlab.df, size = 2, alpha = 0.5,col = 'green') +
...
#fit 
Mdec.fit <- nls_multstart(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Mdec.df,
                                                     iter = 500,
                                                     start_lower = start_vals - 1,
                                                     start_upper = start_vals + 1,
...
# refit model using nlsLM
Mdec.fit_nlsLM <- minpack.lm::nlsLM(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Mdec.df,
                        start = coef(Mdec.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Mdec.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Ik.log10), Mdec.df, size = 2, alpha = 0.5,col = 'cyan') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 Pmax (µmol O2/cm2/hr)')
Mdec.CI.plot 
...
#fit 
Mcav.fit <- nls_multstart(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Mcav.df,
                                                     iter = 500,
                                                     start_lower = start_vals - 1,
                                                     start_upper = start_vals + 1,
...
# refit model using nlsLM
Mcav.fit_nlsLM <- minpack.lm::nlsLM(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Mcav.df,
                        start = coef(Mcav.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Mcav.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Ik.log10), Mcav.df, size = 2, alpha = 0.5,col = 'black') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 Pmax (µmol O2/cm2/hr)')
Mcav.CI.plot 
...
#fit 
Past.fit <- nls_multstart(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Past.df,
                                                     iter = 500,
                                                     start_lower = start_vals - 1,
                                                     start_upper = start_vals + 1,
...
# refit model using nlsLM
Past.fit_nlsLM <- minpack.lm::nlsLM(Ik.log10~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Past.df,
                        start = coef(Past.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Past.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Ik.log10), Past.df, size = 2, alpha = 0.5,col = 'orange') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 Pmax (µmol O2/cm2/hr)')
Past.CI.plot 
...
TPC.plot <- ggplot(data=df, aes()) +
   geom_point(aes(Temp.Cat, Ik.log10), color="green", Dlab.df, size = 2, alpha = 0.5) +
   geom_point(aes(Temp.Cat, Ik.log10), color="cyan", Mdec.df, size = 2, alpha = 0.5) +
   geom_point(aes(Temp.Cat, Ik.log10), color="black", Mcav.df, size = 2, alpha = 0.5) +
   geom_point(aes(Temp.Cat, Ik.log10), color="orange", Past.df, size = 2, alpha = 0.5) +
   geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Dlab.boot1_conf_preds, fill = "green", alpha = 0.3) +
   geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Mdec.boot1_conf_preds, fill = 'cyan', alpha = 0.3) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Mcav.boot1_conf_preds, fill = 'black', alpha = 0.3) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Past.boot1_conf_preds, fill = 'orange', alpha = 0.3) +
   ...
   labs(x = 'Temperature (ºC)',
        y = expression("Log 10 Ik µmol photons "~m^{-2}~s^{-1}))
