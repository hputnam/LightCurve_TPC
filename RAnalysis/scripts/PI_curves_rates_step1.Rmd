...
#test.file<-read_csv(file="data/1_pi_curves/POC-391_20200107.csv", skip=1, col_types = cols(.default = "d", Time = "t")) #do this to correct time format issue
...
metadata <- metadata %>%
  select(Species, colony_id, Run, Chamber.Vol.L, Date, Start.time, Stop.time, Light_Value, Light_Level)
...
# Example of plots
cowplot::plot_grid(df$data_plot[[19]], df$thin_data_plot[[19]], nrow = 2,
...
pr <- left_join(
  ...
  distinct(metadata, Species,colony_id, Run, Chamber.Vol.L)
)
...
blanks %>% ggplot(aes(x=as.numeric(Light_Value), y=micromol.s.blank))+
  geom_point()

str(summary(lm(blanks$micromol.s.blank~as.numeric(blanks$Light_Value))))
anova(lm(blanks$micromol.s.blank~as.numeric(blanks$Light_Value)))

xx <- blanks %>% 
  group_by(Run) %>%
  summarise(avgblank = mean(micromol.s.blank), sdblank  = sd(micromol.s.blank))
...
pr <- pr %>%
  mutate(micromol.s.adj = micromol.s - 0.0002297332) %>%
  # After correcting for blank values, remove blanks from data
  filter(!grepl("BK", colony_id))
...
ggplot(pr, aes(x = as.numeric(Light_Value), y = micromol.cm2.h)) +
  geom_point(size = 3) +
  facet_wrap(~colony_id, scale = "free_y", ncol = 3)
```
...
pr.out <- pr %>% 
  select(Species, colony_id, Light_Value, Run, micromol.cm2.s, micromol.cm2.h) %>%
