...
```{r, warning=FALSE, message=FALSE}
df <- read_csv("data/Phys/PAM_data.csv")
df$Log10.LightY <- log10((df$Light.Yield/100)+1)
df$Log10.DarkY <- log10((df$Dark.Yield/100)+1)

```

Format data columns.  
```{r, warning=FALSE, message=FALSE}
# add a group name by pasting Colony and Temp
...
# Visualize data
light_plot<-df %>%
    ggplot(., aes(x = Temp.Cat, y = Log10.DarkY, colour=species)) +
    geom_point(aes(fill=species, group=species), pch = 21, size=2, alpha=0.5) + 
    xlab("Temperature") + 
    scale_fill_manual(name="species", values=c("green", "cyan", "black", "orange"))+
    scale_color_manual(name="species", values=c("green", "cyan", "black", "orange"))+
    ylab(expression(bold(paste("Light Adapted Fv/Fm")))) +
    ylim(0,1)+
    theme_classic() + 
    theme(legend.position=c(0.2,0.1),
      ...
      legend.text=element_text(size=12)); light_plot

ggsave("output/DarkYield_Scatter_withOutliers.pdf", light_plot, dpi=300, w=8, h=8, units="in")
...
dark_plot<-df %>%
    ...
    scale_fill_manual(name="species", values=c("green", "cyan", "black", "orange"))+
    scale_color_manual(name="species", values=c("green", "cyan", "black", "orange"))+
    ylab(expression(bold(paste("Dark Adapted Fv/Fm")))) +
    ylim(0,1)+
    theme_classic() + 
    theme(legend.position=c(0.2,0.1),
      axis.title=element_text(face="bold", size=16),
      axis.text=element_text(size=12, color="black"), 
      legend.title=element_text(face="bold", size=14), 
      legend.text=element_text(size=12)); dark_plot

ggsave("output/DarkYield_Scatter_withOutliers.pdf", dark_plot, dpi=300, w=8, h=8, units="in")
```

View and remove outliers.   
```{r, warning=FALSE, message=FALSE}
#identify outliers by Temperature and Treatment groups
light.outlier.plot <- ggbetweenstats(df,group, Log10.DarkY, outlier.tagging = TRUE)
ggsave("output/LightYield_outliersbygroup.pdf", light.outlier.plot, dpi=300, w=16, h=8, units="in")

dark.outlier.plot <- ggbetweenstats(df,group, Log10.DarkY, outlier.tagging = TRUE)
ggsave("output/DarkYield_outliersbygroup.pdf", dark.outlier.plot, dpi=300, w=16, h=8, units="in")
...
#   summarize(quant25 = quantile(Ik.log10, probs = q[1]),
...
# x <- df %>%
#   filter(Ik.log10 < upper) %>%
#   filter(Ik.log10 > lower) #%>%
#   #filter(rate < 0.125)
# 
# # Visualize data following outlier removal
# r_plot<-df %>%
#     ggplot(., aes(x = Temp.Cat, y = Ik.log10, colour=species)) +
#     geom_point(aes(fill=species, group=species), pch = 21, size=2, alpha=0.5) + 
#     xlab("Temperature") + 
...
# get start vals
start_vals <- get_start_vals(df$Temp.Cat,df$Log10.DarkY, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(df$Temp.Cat,df$Log10.DarkY, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(df$Temp.Cat,df$Log10.DarkY, model_name = 'sharpeschoolhigh_1981')
...
#fit 
Dlab.fit <- nls_multstart(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Dlab.df,
                                                     iter = 500,
...
Dlab_new_data <- data.frame(temp = seq(min(Dlab.df$Temp.Cat), max(Dlab.df$Temp.Cat), 0.7))
...
Dlab.fit_nlsLM <- minpack.lm::nlsLM(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Dlab.df,
                        start = coef(Dlab.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Dlab.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Log10.DarkY), Dlab.df, size = 2, alpha = 0.5,col = 'green') +
...
Mdec.df <- df %>% 
  ...
  #filter(!id_group=="Mdec-B6_26")

#fit 
Mdec.fit <- nls_multstart(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Mdec.df,
                                                     iter = 500,
                                                     start_lower = start_vals - 1,
                                                     start_upper = start_vals + 1,
...
#generate the predicted data
Mdec_new_data <- data.frame(temp = seq(min(Mdec.df$Temp.Cat), max(Mdec.df$Temp.Cat), 0.7))
...
# refit model using nlsLM
Mdec.fit_nlsLM <- minpack.lm::nlsLM(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Mdec.df,
                        start = coef(Mdec.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Mdec.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Log10.DarkY), Mdec.df, size = 2, alpha = 0.5,col = 'cyan') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 Pmax (µmol O2/cm2/hr)')
Mdec.CI.plot 
...
#fit 
Mcav.fit <- nls_multstart(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Mcav.df,
                                                     iter = 500,
                                                     start_lower = start_vals - 1,
                                                     start_upper = start_vals + 1,
...
#generate the predicted data
Mcav_new_data <- data.frame(temp = seq(min(Mcav.df$Temp.Cat), max(Mcav.df$Temp.Cat), 0.7))
...
# refit model using nlsLM
Mcav.fit_nlsLM <- minpack.lm::nlsLM(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Mcav.df,
                        start = coef(Mcav.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Mcav.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Log10.DarkY), Mcav.df, size = 2, alpha = 0.5,col = 'black') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 Pmax (µmol O2/cm2/hr)')
Mcav.CI.plot 
...
#fit 
Past.fit <- nls_multstart(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                                                     data = Past.df,
                                                     iter = 500,
                                                     start_lower = start_vals - 1,
                                                     start_upper = start_vals + 1,
...
# refit model using nlsLM
Past.fit_nlsLM <- minpack.lm::nlsLM(Log10.DarkY~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = Past.df,
                        start = coef(Past.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
Past.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.Cat, Log10.DarkY), Past.df, size = 2, alpha = 0.5,col = 'orange') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 Pmax (µmol O2/cm2/hr)')
Past.CI.plot 
...
TPC.plot <- ggplot(data=df, aes()) +
   geom_point(aes(Temp.Cat, Log10.DarkY), color="green", Dlab.df, size = 2, alpha = 0.5) +
   geom_point(aes(Temp.Cat, Log10.DarkY), color="cyan", Mdec.df, size = 2, alpha = 0.5) +
   geom_point(aes(Temp.Cat, Log10.DarkY), color="black", Mcav.df, size = 2, alpha = 0.5) +
   geom_point(aes(Temp.Cat, Log10.DarkY), color="orange", Past.df, size = 2, alpha = 0.5) +
   geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Dlab.boot1_conf_preds, fill = "green", alpha = 0.3) +
   geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Mdec.boot1_conf_preds, fill = 'cyan', alpha = 0.3) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Mcav.boot1_conf_preds, fill = 'black', alpha = 0.3) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), Past.boot1_conf_preds, fill = 'orange', alpha = 0.3) +
   ...
   labs(x = 'Temperature (ºC)',
        y = expression("Dark Adapted Fv/Fm"))

TPC.plot 

ggsave("output/TPC_SharpSchool_Dark_Yeild.pdf", TPC.plot, dpi=300, w=8, h=8, units="in")
